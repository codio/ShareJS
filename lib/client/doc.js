...
Doc.prototype._otApply = function(opData, context) {
  ...
  if (opData.create) {
    ...
    // Add the inverted operation to the undo manger if it's a local operation
    if (context) this.undoManager.pushUndo(invert(this.type, this.snapshot, op));
 ...
 * Undo
 */
Doc.prototype.undo = function(context) {
  ...
  var op = manager.undo();
  console.log(op);
  console.log(invert(this.type, this.snapshot, op));

  // Save for redo operations.
  manager.pushRedo(invert(this.type, this.snapshot, op));

  // Apply the operation on the given context.
  this._otApply({op: op}, context);
};
 ...
 * Redo
 */
Doc.prototype.redo = function(context) {
  var manager = this.undoManager;
  ...
  manager.pushUndo(invert(this.type, this.snapshot, op));
