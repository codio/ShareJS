...
Doc.prototype._otApply = function(opData, context) {
  ...
  if (opData.create) {
    ...
    // If it's a non local operation tranform the undo stacks
    if (!context) this.undoManager.transform(op, this.type);
...
Doc.prototype._submitOpData = function(opData, context, callback) {
  ...
  // Add the inverted operation to the undo manger
  if (this.type) this.undoManager.pushUndo(invert(this.type, this.snapshot, opData.op));
...
Doc.prototype.createContext = function() {
  ...
  var context = {
    ...
    undo: function(callback) {
      doc.undo(this, callback);
    },
    redo: function(callback) {
      doc.redo(this, callback);
    },
...
/*
 * Invert a given operation based on the supplied type and the snapshot.
 * The snapshot provided is before the operation was supplied.
 */
function invert(type, snapshot, op) {
  if (typeof type.invert === 'function') {
    return type.invert(op);
  } else if (typeof type.semanticInvert === 'function') {
    return type.semanticInvert(snapshot, op);
  }
  throw new Error(type.name + ' has no way of inversion.');
}

/*
 * Undo
 */
Doc.prototype.undo = function(context, callback) {
  var manager = this.undoManager;

  // If we can't undo just ignore.
  if (!manager.canUndo()) return;

  var self = this;

  // Get the operation to be applied.
  manager.undo(function (err, op) {
    if (err) throw err;

    // Save for redo operations.
    manager.pushRedo(invert(self.type, self.snapshot, op));

    // Apply the operation on the given context.
    self.submitOp(op, context, callback);
  });
};
 ...
 * Redo
 */
Doc.prototype.redo = function(context, callback) {
  var manager = this.undoManager;

  // If we can't redo just ignore.
  if (!manager.canRedo()) return;

  var self = this;

  // Get the operation to be applied.
  manager.redo(function (err, op) {
    if (err) throw err;

    // Save for undo operations.
    manager.pushUndo(invert(self.type, self.snapshot, op));
