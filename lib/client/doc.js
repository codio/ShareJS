...
Doc.prototype.ingestData = function(data) {
  ...
  if (this.state) {
    ...
    this.emit('warn', 'Ignoring ingest data for', this.collection, this.name,
  ...
  switch (msg.a) {
    ...
    case 'ack':
      ...
      if (msg.error && msg.error !== 'Op already submitted') {
        ...
        if (this.inflightData) {
          this.emit('warn', 'Operation was rejected (' + msg.error + '). Trying to rollback change locally.');
          ...
          this.emit('warn', 'Second acknowledgement message (error) received', msg);
        }
      }
    ...
    case 'op':
      ...
      if (msg.v > this.version) {
        ...
        this.emit('warn', 'Client got future operation from the server',
    ...
    case 'meta':
      this.emit('warn', 'Unhandled meta op:', msg);
      break;

    default:
      this.emit('warn', 'Unhandled document message:', msg);
      break;
  }
};
...
Doc.prototype._otApply = function(opData, context) {
  ...
  if (opData.op) {
    ...
    for (var i = 0; i < contexts.length; i++) {
      if (contexts[i]._shouldRemove) contexts.splice(i--, 1);
    }
...
Doc.prototype._submitOpData = function(opData, context, callback) {
  ...
  var error = function(err) {
    ...
    this.emit('warn', 'Failed attempt to submitOp:', err);
  };
...
Doc.prototype._tryRollback = function(opData) {
  ...
  if (opData.create) {
    ...
    if (this.state === 'floating')
      ...
      this.emit('warn', 'Rollback a create from state ' + this.state);
