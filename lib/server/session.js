...
Session.prototype._setSubscribed = function(c, docName, value, v) {
  ...
  if (!value) {
      this.agent.trigger('unsubscribe', c, docName, function(){});
  }
  docs[docName] = value;
...
Session.prototype.bulkSubscribe = function(request, callback) {
  ...
  var cancel = function() {
    for (var cName in request) {
      var docs = request[cName];
      for (var docName in docs) {
        self._setSubscribed(cName, docName, false);
      }
    }
  };
...
Session.prototype._send = function(msg) {
  ...
  try {
    ...
    // Fix race condition where the cleanup for the stream
    // hasn't happen yet and we are trying to write to it.
    if (e.message === 'write after end') {
      return this._cleanup();
    }
    console.warn('Could not write to stream', e.stack);
  }
};
  ...
  for (var i = 0; i < results.length; i++) {
    ...
    if (qopts.docMode && !this._isSubscribed(collection, docName)) {
      ...
      if (qopts.docMode === 'sub') {
        ...
        (function(collection, docName) {
          var v = atVersion || r.v;
          self.agent.subscribe(collection, docName, v, function(err, opstream) {
            if (err) {
              self._setSubscribed(collection, docName, false);
              self._send({a:'sub', c:collection, d:docName, error:err});
              return;
            }
